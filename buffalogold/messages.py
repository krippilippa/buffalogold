...
from buffalogold.schema import Peer, Block, Transaction, Ping
...
def meta(ip, port, version="buffalogold-0.1"):
    return{
        "client": version,
        "address": {
            "ip": ip,
...
def create_ping_message(external_ip, external_port, block_height, peer_count, temp):
    return BaseSchema().dumps({
        "meta": meta(external_ip, external_port),
        "message": {
            "name": "ping",
            "payload": (block_height, peer_count, temp)
        }
    })
